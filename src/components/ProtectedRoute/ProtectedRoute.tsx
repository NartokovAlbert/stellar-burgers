...
import { userSelectors } from 'src/services/userSlice';
...
type ProtectedRouteProps = {
  ...
  export const ProtectedRoute = ({ onlyUnAuth, children }: ProtectedRouteProps) => {
    ...
    if (!onlyUnAuth && !user) { // если пользователь на странице авторизации и данных в хранилище нет, то делаем редирект
      return <Navigate replace to='/login' state={{ from: {...location, backgroundLocation: location.state?.backgroundLocation, state: null}}} />; // в поле from объекта location.state записываем информацию о URL
    }
